...
pokerHand.prototype.highCard = function() {
    return Math.max.call(pokerHand, this.card[0].rankValue, this.card[1].rankValue, this.card[2].rankValue, this.card[3].rankValue, this.card[4].rankValue);
};
...
pokerHand.prototype.hasSets = function() {
    ...
    for (var cardRank in handSets) {
        ...
        if (handSets[cardRank] === 2) {
            if (sets === "Three of a Kind") { sets = "Full House"; } else if (sets === "Pair") { sets = "Two Pair"; } else {
                sets = "Pair";
                pairRank = cardRank;
            }
        }
